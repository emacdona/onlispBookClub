...
Let's say you do your banking with (**origin**) `https://www.mybank.example.com`{:.language-shell .highlight}. They've got a modern website: a JavaScript frontend app that makes calls to a web service backend. The frontend authenticates you, stores your credentials in a cookie, and sends that cookie with every request it makes to the backend. One of those backend services is `/transfer_to/<routing_number>/<account_number>/<amount>`{:.language-shell .highlight}.

Actually, that's not quite accurate. Since your credentials are in a cookie, the frontend app doesn't actively send them. It relies on the browser to send the cookie with every XMLHttpRequest it makes to the backend web services. That's an important distinction: if your browser stored a cookie for `www.mybank.example.com`{:.language-shell .highlight}, it will send that cookie for every request it makes to `www.mybank.example.com`{:.language-shell .highlight}.

So let's say you go to your bank, log in, and use their bill payment system to send a rent check to your landlord. Having taken care of that, it's time to crack open a beer and watch a movie. So, after opening that beer, you go to your favorite source of online content, `https://www.shadymovietorrenting.example.com`{:.language-shell .highlight}.
...
And guess what? When you open `https://www.shadymovietorrenting.example.com`{:.language-shell .highlight}, it makes this XMLHttpRequest:
